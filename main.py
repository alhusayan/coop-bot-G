...
app = FastAPI()
BUILD_ID = "v84-LENS-DIRECT-v71-HIS-CODE-8-results-soft-stock-no-msg-20260814"
...
def send_lens_direct_results(from_number, lens, bot_id, lang, caption=""):
    ...
    if not matches:
        return False
    # v84 SOFT STOCK FILTER - احذف فقط اللي واضح جداً out of stock من العنوان
    def _is_out_title(m):
        hay = (str(m.get("title") or "") + " " + str(m.get("snippet") or "")).lower()
        return any(p in hay for p in ["out of stock", "currently out of stock", "sold out", "غير متوفر", "نفدت الكمية"])
    matches = [m for m in matches if not _is_out_title(m)]
    # v84 HIGH COMMISSION PRIORITY - رتب المتاجر عالية العمولة أولاً
    def _comm_score(m):
        url = (m.get("link") or "").lower()
        order = ["temu.com","shein.com","trendyol.com","namshi.com","ounass.com","6thstreet.com","farfetch.com","amazon.ae","amazon.sa","noon.com","aliexpress.com"]
        for i, d in enumerate(order):
            if d in url:
                return i
        return 99
    matches = sorted(matches, key=_comm_score)
    if not matches:
        return False
