...
# تحديث برومبت التعرف البصري ليكون أكثر دقة مثل جوجل لينس
IDENTIFY_SYSTEM = """أنت خبير فحص المنتجات ومطابقتها البصرية.
انظر للصورة واستخرج الاسم الدقيق جداً للمنتج والموديل ونوع التصميم الخارجي بصيغة ثابتة:
[Brand] [Product Type & Model Name] [Specific Design/Material/Pattern] [Color]

أمثلة صارمة:
- Bottega Veneta Intrecciato woven leather flat mules brown
- Apple Watch Ultra 2 Titanium Green Alpine Loop
- Ray-Ban RB3721 Aviator sunglasses black 59mm
- Pringles Ketchup 200g

قاعدة صارمة: اكتب الوصف باللغة الإنجليزية دائماً (لضمان دقة البحث في المتاجر) وركز بشكل أساسي على الموديل والنقشة الدقيقة (مثل Intrecciato).
سطر واحد فقط."""
...
# إضافة دالة التحقق من تطابق عنوان الصفحة مع الموديل لتفادي اقتراح سنيكرز بدل Mules
def is_title_matching(query, page_title):
    q_lower = (query or "").lower()
    t_lower = (page_title or "").lower()
...
def verify_offers(urls_map, query):
    ...
    def _check(item):
        ...
        # التحقق من أن المنتج في الصفحة يطابق طلبنا الدقيق وليس منتجاً آخر من نفس الماركة
        if not is_title_matching(query, info["title"]):
            print(f"REJECT TITLE MISMATCH: {query} != {info['title']} -> {url}")
            return None
...
# تعديل دالة الصورة لطلب بحث صارم بالموديل الدقيق 
def process_single_image(message,bot_id,lang="ar"):
    ...
    ident,_=call_gemini([{"inline_data":{"mime_type":mime,"data":b64}},{"text":"Extract the exact product name, model, and design."}], system=IDENTIFY_SYSTEM, use_search=False)
    
    product_name = ident.strip().splitlines()[0].strip() if ident else ""
    
    if product_name and caption:
        request_query = product_name
        prompt_text = f"المنتج الدقيق في الصورة هو: '{product_name}'\nطلب المستخدم: {caption}\nابحث حصرياً عن هذا الموديل بالذات في الكويت. لا تقترح أي موديلات أخرى أو تصاميم مختلفة لنفس الماركة. {LANG_INSTR[lang]}"
        txt,urls=search_product(request_query, lang, prompt_text=prompt_text)
        LAST_SEARCH[from_number] = {"product": request_query}
        query = request_query
    elif product_name:
        prompt_text = f"ابحث حصرياً عن الموديل الدقيق: '{product_name}' متوفر في الكويت. يمنع منعاً باتاً اقتراح موديلات أخرى من نفس الماركة. {LANG_INSTR[lang]}"
        txt,urls=search_product(product_name, lang, prompt_text=prompt_text)
...
@app.get("/")
async def health(): return {"status":"v32 verified-prices - precision title matching"}
