...
app = FastAPI()
BUILD_ID = "v79-global-plus-20260820"
print("=" * 70)
...
LANGUAGE_LOCALES = {
    "ar":"ar","en":"en","de":"de","fr":"fr","es":"es","it":"it",
    "tr":"tr","pt":"pt","ja":"ja","ko":"ko","zh":"zh",
}
COUNTRY_DEFAULT_LANGUAGE = {
    "kw":"ar","sa":"ar","ae":"ar","bh":"ar","qa":"ar","om":"ar","iq":"ar","jo":"ar","lb":"ar","eg":"ar",
    "ma":"ar","dz":"ar","tn":"ar","ly":"ar","sd":"ar","sy":"ar","ye":"ar","ps":"ar",
    "de":"de","at":"de","li":"de","fr":"fr","mc":"fr","es":"es","it":"it","tr":"tr",
    "pt":"pt","br":"pt","jp":"ja","kr":"ko","cn":"zh","tw":"zh","hk":"zh","mo":"zh",
}

# Merchant + market -> affiliate network/template.
# No config means the original direct product URL is used unchanged.
try:
    AFFILIATE_ROUTES = json.loads(os.environ.get("AFFILIATE_ROUTES_JSON", "{}") or "{}")
    if not isinstance(AFFILIATE_ROUTES, dict):
        AFFILIATE_ROUTES = {}
except Exception as _e:
    print(f"AFFILIATE_ROUTES_JSON parse error: {_e}")
    AFFILIATE_ROUTES = {}
...
ZERO_DECIMAL_CURRENCIES = {
    "JPY","KRW","VND","IDR","CLP","PYG","RWF","UGX","VUV","XAF","XOF","XPF",
    "DJF","GNF","KMF","MGA","BIF",
}
CURRENCY_MINOR_UNITS = {
    **{c: 3 for c in THREE_DECIMAL_CURRENCIES},
    **{c: 0 for c in ZERO_DECIMAL_CURRENCIES},
}
...
_AR_DIGIT_TRANS_GLOBAL = str.maketrans("٠١٢٣٤٥٦٧٨٩۰۱۲۳۴۵۶۷۸۹", "01234567890123456789")

def _price_decimal(value):
    """Parse the source amount without binary-float precision loss."""
    if isinstance(value, Decimal):
        return value
    s = str(value or "").translate(_AR_DIGIT_TRANS_GLOBAL)
    ...
    if "," in tok and "." in tok:
        ...
        bits = tok.split(",")
        if len(bits) == 2 and len(bits[1]) in (1, 2):
            tok = tok.replace(",", ".")
        else:
            tok = tok.replace(",", "")
    try:
...
def _currency_decimals(code):
    return CURRENCY_MINOR_UNITS.get((code or "").upper().strip(), 2)

def _format_decimal_money(value, currency=None):
    d = _price_decimal(value)
    if d is None:
        return str(value or "")
    code = (currency or current_market().get("currency") or "KWD").upper().strip()
    places = _currency_decimals(code)
    quantum = Decimal(1).scaleb(-places)
    # Only quantize to the currency's actual minor unit (fils/cents/yen).
...
def display_global_price(price_value, price_text, currency_code, lang="ar"):
    """Converted local price at full minor-unit precision + exact original price."""
    src = detect_currency_code(f"{currency_code or ''} {price_text or ''}", currency_code)
    numeric = _price_decimal(price_value)
    if numeric is None:
        numeric = _price_decimal(price_text)
    if numeric is None:
    ...
    if converted is not None:
        ...
        return f"{format_price(converted, local_code)} {currency_label(lang)}{original}", converted
...
def build_market_profile(cc, source="saved", base=None):
    cc = (cc or DEFAULT_COUNTRY).lower()
    market = dict(base or {})
    market["country"] = cc
    market["country_name"] = COUNTRY_NAMES.get(cc, cc.upper())
    market["currency"] = COUNTRY_CURRENCIES.get(cc, market.get("currency") or "USD")
    market["country_source"] = source
    lang = market.get("language") or COUNTRY_DEFAULT_LANGUAGE.get(cc, "en")
    market["locale"] = market.get("locale") or f"{LANGUAGE_LOCALES.get(lang, lang)}-{cc.upper()}"
    return market

def market_for_user(from_number):
    market = dict(USER_MARKET.get(from_number) or {})
    if market.get("country"):
        return build_market_profile(market["country"], market.get("country_source") or "saved", market)
    return build_market_profile(infer_country_from_phone(from_number), "phone_prefix", market)

def set_manual_market(phone, cc):
    market = build_market_profile(cc, "manual", USER_MARKET.get(phone) or {})
    market.pop("lat", None); market.pop("lng", None)
    USER_MARKET[phone] = market
    USER_LOCATION_TS[phone] = time.time()
...
def current_market():
    return getattr(MARKET_CTX, "value", None) or build_market_profile(DEFAULT_COUNTRY, "default")
...
def format_price(p, currency=None):
    """Full minor units: 48.374 KWD, 157.00 USD, 1250 JPY."""
    return _format_decimal_money(p, currency)
...
def format_lens_price(price_text, price_value, lang="ar", currency_code=None):
    ...
    code = (currency_code or detect_currency_code(price_text, "") or current_market().get("currency") or "").upper()
    return f"{format_price(numeric, code or None)} {currency_label(lang)}"
...
def cache_key(query, lang):
    ...
    return hashlib.sha256(f"v79-global-plus|{market}|{norm}|{lang}".encode()).hexdigest()
...
def location_is_valid(phone):
    ...
    if market.get("country_source") == "manual" and market.get("country"):
        return True
    return bool(
        market.get("country") and market.get("lat") is not None and market.get("lng") is not None
        and (time.time() - ts) < LOCATION_TTL_SECONDS
    )
...
# Additional localized UI. Missing keys fall back to English.
MSG.update({
 "de":{"identifying":"✨ Einen Moment.. ich erkenne das Produkt und suche die besten Optionen.","searching":"🔎 Ich suche nach {q}...","not_found":"Kein passendes verfügbares Angebot mit bestätigtem Preis gefunden.","welcome_reply":"Hallo! 🌟 Sende ein Produktfoto oder den Namen, und ich suche Preise und Shops.","thanks_reply":"Gern! 🌹 Schick mir das nächste Produkt.","compare_searching":"⚖️ Die Anfrage ist allgemein – ich vergleiche zuerst die besten Optionen.","pick_prompt":"Wähle ein Produkt und ich suche die besten Preise 👇","list_button":"Produkt wählen"},
 "fr":{"identifying":"✨ Un instant.. j’identifie le produit et cherche les meilleures options.","searching":"🔎 Je cherche {q}...","not_found":"Aucune offre correspondante disponible avec prix confirmé.","welcome_reply":"Bonjour ! 🌟 Envoyez une photo ou le nom du produit et je chercherai les prix et boutiques.","thanks_reply":"Avec plaisir ! 🌹 Envoyez le prochain produit.","compare_searching":"⚖️ Votre demande est générale – je compare d’abord les meilleures options.","pick_prompt":"Choisissez un produit et je chercherai les meilleurs prix 👇","list_button":"Choisir"},
 "es":{"identifying":"✨ Un momento.. identifico el producto y busco las mejores opciones.","searching":"🔎 Buscando {q}...","not_found":"No encontré una oferta coincidente con precio confirmado.","welcome_reply":"¡Hola! 🌟 Envía una foto o el nombre del producto y buscaré precios y tiendas.","thanks_reply":"¡Con gusto! 🌹 Envíame el siguiente producto.","compare_searching":"⚖️ Tu solicitud es general; primero comparo las mejores opciones.","pick_prompt":"Elige un producto y buscaré los mejores precios 👇","list_button":"Elegir"},
 "it":{"identifying":"✨ Un momento.. identifico il prodotto e cerco le opzioni migliori.","searching":"🔎 Cerco {q}...","not_found":"Non ho trovato un’offerta con prezzo confermato.","welcome_reply":"Ciao! 🌟 Invia una foto o il nome del prodotto e cercherò prezzi e negozi.","thanks_reply":"Volentieri! 🌹 Mandami il prossimo prodotto.","compare_searching":"⚖️ La richiesta è generica: confronto prima le opzioni migliori.","pick_prompt":"Scegli un prodotto e cercherò i prezzi migliori 👇","list_button":"Scegli"},
 "tr":{"identifying":"✨ Bir saniye.. ürünü belirleyip en iyi seçenekleri arıyorum.","searching":"🔎 {q} aranıyor...","not_found":"Doğrulanmış fiyatlı uygun sonuç bulamadım.","welcome_reply":"Merhaba! 🌟 Ürün fotoğrafını veya adını gönder, fiyat ve mağaza bulayım.","thanks_reply":"Rica ederim! 🌹 Sıradaki ürünü gönder.","compare_searching":"⚖️ İstek genel; önce en iyi seçenekleri karşılaştırıyorum.","pick_prompt":"Bir ürün seç, en iyi fiyatları arayayım 👇","list_button":"Ürün seç"},
 "pt":{"identifying":"✨ Um momento.. identificando o produto e buscando as melhores opções.","searching":"🔎 Buscando {q}...","not_found":"Não encontrei oferta correspondente com preço confirmado.","welcome_reply":"Olá! 🌟 Envie uma foto ou o nome do produto e buscarei preços e lojas.","thanks_reply":"De nada! 🌹 Envie o próximo produto.","compare_searching":"⚖️ Seu pedido é geral; primeiro comparo as melhores opções.","pick_prompt":"Escolha um produto e buscarei os melhores preços 👇","list_button":"Escolher"},
 "ja":{"identifying":"✨ 少々お待ちください。商品を特定して最適な選択肢を探しています。","searching":"🔎 {q} を検索しています...","not_found":"価格を確認できる一致商品が見つかりませんでした。","welcome_reply":"こんにちは！🌟 商品写真または商品名を送ってください。価格とショップを探します。","thanks_reply":"どういたしまして！🌹 次の商品を送ってください。","compare_searching":"⚖️ 一般的なご依頼なので、まず最適な候補を比較します。","pick_prompt":"商品を選ぶと最適な価格を検索します 👇","list_button":"商品を選択"},
 "ko":{"identifying":"✨ 잠시만요.. 상품을 확인하고 최적의 옵션을 찾고 있습니다.","searching":"🔎 {q} 검색 중...","not_found":"확인된 가격이 있는 일치 상품을 찾지 못했습니다.","welcome_reply":"안녕하세요! 🌟 상품 사진이나 이름을 보내주시면 가격과 매장을 찾아드립니다.","thanks_reply":"천만에요! 🌹 다음 상품을 보내주세요.","compare_searching":"⚖️ 일반 요청이라 먼저 가장 좋은 옵션을 비교합니다.","pick_prompt":"상품을 선택하면 최적의 가격을 찾아드립니다 👇","list_button":"상품 선택"},
 "zh":{"identifying":"✨ 稍等，我正在识别商品并寻找最佳选择。","searching":"🔎 正在搜索 {q}...","not_found":"目前没有找到带有已确认价格的匹配商品。","welcome_reply":"你好！🌟 发送商品照片或名称，我会帮你查找价格和商店。","thanks_reply":"不客气！🌹 发下一个商品给我即可。","compare_searching":"⚖️ 你的需求比较宽泛，我先比较最佳选项。","pick_prompt":"选择一个商品，我会查找最佳价格 👇","list_button":"选择商品"}
})
...
for _lc in SUPPORTED_LANGUAGES:
    LANG_INSTR.setdefault(_lc, LANG_INSTR["en"])


def T(lang, key, **kw):
    table = MSG.get(lang) or {}
    value = table.get(key)
    if value is None:
...
def detect_lang(text):
    ...
    signals = {
        "fr":(" le "," la "," les "," une "," pour "," avec "," meilleur "," prix "),
        "es":(" el "," los "," una "," para "," con "," mejor "," precio "),
        "it":(" il "," lo "," gli "," una "," per "," con "," migliore "," prezzo "),
        "pt":(" os "," as "," uma "," para "," com "," melhor "," preço "),
        "de":(" der "," die "," das "," ein "," eine "," für "," mit "," beste "," preis "),
        "tr":(" bir "," için "," ile "," en iyi "," fiyat "),
        "en":(" the "," a "," an "," for "," with "," best "," price "," buy "),
    }
...
def _build_compact_card_body(flag, store, title, price_text, lang="ar"):
    lines = []
    # Entire flag + merchant line is bold.
    header = f"*{flag} {store}*" if store else f"*{flag}*"
    if header.strip("* "):
        lines.append(header.strip())

    # Product/category + brand/model are NORMAL text. No italics here.
    for tline in _single_direction_lines(_compact_ui_title(title or ""), lang, max_groups=3):
        if tline.strip():
...
def _affiliate_route_for(host, country):
    host = (host or "").lower().replace("www.", "")
    country = (country or "").lower()
    keys = [host]
    parts = host.split(".")
    if len(parts) > 2: keys.append(".".join(parts[-2:]))
    keys.append("*")
    for key in keys:
        spec = AFFILIATE_ROUTES.get(key)
        if not isinstance(spec, dict): continue
        route = spec.get(country) or spec.get("*")
        if route: return route
    return None

def apply_affiliate_tracking(raw_url, phone):
    url = str(raw_url or "").strip()
    if not url.startswith(("http://","https://")) or not AFFILIATE_ROUTES:
        return url
    try:
        host = urllib.parse.urlparse(url).netloc.lower().split(":")[0].replace("www.","")
        cc = (market_for_user(phone).get("country") or DEFAULT_COUNTRY).lower()
        route = _affiliate_route_for(host, cc)
        if not route: return url
        if isinstance(route, str):
            template, network = route, ""
        elif isinstance(route, dict):
            template, network = str(route.get("template") or ""), str(route.get("network") or "")
        else:
            return url
        if not template: return url
        tracked = template.format(url=url,url_encoded=urllib.parse.quote_plus(url),country=cc,merchant=host)
        if tracked.startswith(("http://","https://")):
            print(f"AFFILIATE ROUTE market={cc} merchant={host} network={network or 'configured'}")
            return tracked
    except Exception as e:
        print(f"AFFILIATE ROUTE ERR: {e}")
    return url

VIEW_PRODUCT_LABELS = {
 "ar":"🛒 عرض المنتج","en":"View product","de":"Produkt ansehen","fr":"Voir produit",
 "es":"Ver producto","it":"Vedi prodotto","tr":"Ürünü gör","pt":"Ver produto",
 "ja":"商品を見る","ko":"상품 보기","zh":"查看商品",
}
def _view_product_label(lang):
    return VIEW_PRODUCT_LABELS.get(lang, VIEW_PRODUCT_LABELS["en"])
...
def send_whatsapp_cta(to,body,link,bot_id,title):
    ...
    tracked_link = apply_affiliate_tracking(link, to)
    payload={"messaging_product":"whatsapp","to":to,"type":"interactive","interactive":{"type":"cta_url","body":{"text":safe_body[:1024]},"action":{"name":"cta_url","parameters":{"display_text":safe_title[:20],"url":tracked_link}}}}
    try: return requests.post(url,json=payload,headers=h,timeout=15).ok
    except: return False
...
def send_language_choice(to, bot_id):
    rows = [
        {"id":f"lang_{code}","title":meta[0],"description":meta[1]}
        for code, meta in list(SUPPORTED_LANGUAGES.items())[:10]
    ]
    return send_whatsapp_list(to, "🌐 Choose your language / اختر اللغة", rows, bot_id, "Language")
...
MORE_RESULTS_LABELS = {
 "ar":"🔎 نتائج أكثر","en":"🔎 More results","de":"🔎 Mehr Ergebnisse","fr":"🔎 Plus de résultats",
 "es":"🔎 Más resultados","it":"🔎 Altri risultati","tr":"🔎 Daha fazla","pt":"🔎 Mais resultados",
 "ja":"🔎 さらに表示","ko":"🔎 더 보기","zh":"🔎 更多结果",
}
MORE_RESULTS_BODIES = {
 "ar":"✨ تبي خيارات أكثر لنفس المنتج؟","en":"✨ Want more options for the same product?",
 "de":"✨ Mehr Optionen für dasselbe Produkt?","fr":"✨ Plus d’options pour le même produit ?",
 "es":"✨ ¿Más opciones del mismo producto?","it":"✨ Altre opzioni per lo stesso prodotto?",
 "tr":"✨ Aynı ürün için daha fazla seçenek?","pt":"✨ Mais opções para o mesmo produto?",
 "ja":"✨ 同じ商品の候補をもっと表示しますか？","ko":"✨ 같은 상품의 옵션을 더 볼까요?","zh":"✨ 查看同一商品的更多选项？",
}
def _send_more_results_choice(phone, bot_id, lang):
    return send_whatsapp_buttons(
        phone,
        MORE_RESULTS_BODIES.get(lang, MORE_RESULTS_BODIES["en"]),
        [{"id":"more_results","title":MORE_RESULTS_LABELS.get(lang, MORE_RESULTS_LABELS["en"])}],
        bot_id,
    )

def _save_more_state(phone, query, bot_id, lang, origin, shown_items,
                     image_b64="", image_mime="", visual_identity="", reset=False):
    prev = {} if reset else (PENDING_MORE_RESULTS.get(phone) or {})
    domains, urls = set(prev.get("seen_domains") or []), set(prev.get("seen_urls") or [])
    for item in shown_items or []:
        url = str((item or {}).get("link") or "").strip()
        if url:
            urls.add(url)
            dom = _result_domain(url)
    ...
    PENDING_MORE_RESULTS[phone] = {
        ...
        "seen_domains":sorted(domains),"seen_urls":sorted(urls),"ts":time.time(),
    }

def _text_more_search(product, lang, seen_domains):
    market_name = current_market().get("country_name", "Kuwait")
    cap = max(1, LENS_DIRECT_LOCAL_MAX + LENS_DIRECT_US_MAX + LENS_DIRECT_CN_MAX)
    excluded = ", ".join(list(seen_domains or [])[:20])
    prompt = (
        f"ابحث مرة ثانية بعمق عن نفس المنتج بالضبط: {product}. "
        f"لا تكرر هذه المواقع التي ظهرت سابقاً: {excluded}. "
        f"متاجر جديدة فقط، بنفس الترتيب: {market_name} محلياً ثم الولايات المتحدة ثم الصين. "
        f"الحدود نفسها: {LENS_DIRECT_LOCAL_MAX} محلي، {LENS_DIRECT_US_MAX} أمريكا، {LENS_DIRECT_CN_MAX} الصين. "
        "لا دولة رابعة. نفس المنتج/الموديل/الحجم فقط. كل نتيجة بسعر رقمي حالي ورابط منتج مباشر. "
        f"{TEXT77_LANG_INSTR.get(lang, TEXT77_LANG_INSTR['en'])}"
    )
    return legacy_v26_best_of_search([{"text":prompt}], max_results=cap, merge_offers=True, merge_title=product)
...
def run_more_results_search(phone, item):
    ...
    domains, urls = set(item.get("seen_domains") or []), set(item.get("seen_urls") or [])
    ok = False
    send_whatsapp_text(phone, "🔎 أدور لك على خيارات جديدة..." if lang=="ar" else "🔎 Looking for new options...", bot_id)
    if item.get("origin") == "lens" and item.get("image_b64") and item.get("image_mime"):
        ex = " ".join(f"-site:{d}" for d in list(domains)[:5])
        hint = re.sub(r"\s+"," ",f"{query} {ex}").strip()[:150]
        lens = google_lens_lookup(item["image_b64"], item["image_mime"], lang, hint, light=True)
        if lens.get("matches"):
            ok = send_lens_direct_results(
                phone,lens,bot_id,lang,caption=query,
                image_b64=item["image_b64"],image_mime=item["image_mime"],
                exclude_domains=domains,exclude_urls=urls,more_mode=True
            )
    else:
        txt, found_urls = _text_more_search(query, lang, domains)
        if txt and found_urls:
            ok = send_text_lens_style_results(
                phone,txt,found_urls,bot_id,lang,query,
                exclude_domains=domains,exclude_urls=urls,more_mode=True
            )
    if not ok:
        ...
        send_whatsapp_text(
            phone,
            "✅ هذي تقريباً كل النتائج المطابقة اللي قدرت ألقاها حالياً." if lang=="ar"
...
def process_interactive_message(message, bot_id):
    ...
    if btn_id.startswith("pick_"):
        ...
        if picked:
            ...
            print(f"BRAND PICK CONTEXT: {picked!r} + {original_query!r} -> {detailed_query!r}")
            execute_product_search(from_number, detailed_query, item.get("bot_id") or bot_id, lang_)
        else:
    ...
    mkt = market_for_user(from_number)
    mkt["language"] = lang
    mkt["locale"] = f"{LANGUAGE_LOCALES.get(lang,lang)}-{mkt['country'].upper()}"
    USER_MARKET[from_number] = mkt
    save_user_preferences(from_number)
    if location_is_valid(from_number):
...
def translate_ui_titles(titles, lang):
    ...
    if not clean or lang == "en":
        return clean
    target = SUPPORTED_LANGUAGES.get(lang, ("English","English"))[1]
...
PRICE_RECOVERY_SEARCH_MAX = max(1, int(os.environ.get("PRICE_RECOVERY_SEARCH_MAX", "6")))
PRICE_RECOVERY_TOTAL_TIMEOUT = max(5, int(os.environ.get("PRICE_RECOVERY_TOTAL_TIMEOUT", "18")))

def _item_has_price(item):
    return bool(
        str((item or {}).get("price") or "").strip()
        or (item or {}).get("price_value") not in (None, "")
    )

def _recover_selected_prices(items, target, lang):
    seq = [dict(x) for x in (items or [])]
    if not seq: return seq

    # 1) Direct product page, using v79's existing JSON-LD/meta parser.
    seq = _enrich_missing_prices(seq)
    missing = [i for i,x in enumerate(seq) if not _item_has_price(x)]
    if not missing: return seq

    # 2) Focused grounded lookup for that exact merchant/product.
    market_snapshot = current_market()
    jobs = {}
    for i in missing[:PRICE_RECOVERY_SEARCH_MAX]:
        fut = RESOLVER.submit(
            _run_with_market, market_snapshot,
            _lens_price_from_text_search_one, seq[i], target, lang
        )
        jobs[fut] = i
    done, pending = wait(list(jobs), timeout=PRICE_RECOVERY_TOTAL_TIMEOUT)
    for fut in done:
        i = jobs[fut]
        try: seq[i] = fut.result()
        except Exception as e: print(f"PRICE RECOVERY ERR idx={i}: {e}")
    for fut in pending: fut.cancel()
    print(f"PRICE RECOVERY: {sum(_item_has_price(x) for x in seq)}/{len(seq)} priced")
    return seq
...
"""
    ...
    mm = re.search(
        r"PRICE\s*:\s*((?:[A-Z]{3}|US\$|\$|€|£|¥|￥)?\s*\d[\d,.]*\s*(?:[A-Z]{3}|US\$|\$|€|£|¥|￥)?)",
        raw, flags=re.I,
    )
...
def send_lens_direct_results(from_number, lens, bot_id, lang, caption="", image_b64="", image_mime="", exclude_domains=None, exclude_urls=None, more_mode=False):
    ...
    if exclude_domains or exclude_urls:
        raw_matches = [
            ...
            and _result_domain(m.get("link")) not in exclude_domains
        ]
    ...
    _price_target = str(
        (lens.get("relevance_target") or caption or ((lens.get("chosen") or {}).get("title"))
         or selected[0].get("title") or "")
    ).strip()
    selected = _recover_selected_prices(selected, _price_target, lang)
    selected = [m for m in selected if _item_has_price(m)]
    if not selected:
        return False
    ...
    for m in selected:
        ...
        send_whatsapp_cta(from_number, body[:1000], url, bot_id, _view_product_label(lang))
        sent += 1
    ...
    if sent > 0 and expansion_query:
        _save_more_state(
            from_number, expansion_query, bot_id, lang, "lens", selected,
...
for _lc in SUPPORTED_LANGUAGES:
    TEXT77_LANG_INSTR.setdefault(_lc, TEXT77_LANG_INSTR["en"])
...
def run_region_lens_search(phone, product, region_key, bot_id, lang,
    ...
    for m, shown_title in zip(selected, display_titles):
        ...
        send_whatsapp_cta(
            ...
            bot_id, _view_product_label(lang)
        )
        sent += 1
...
def run_region_search(phone, product, region_key, bot_id, lang="ar", origin="text",
    ...
    for item, display_title in zip(selected, display_titles):
        ...
        body = _build_compact_card_body(flag, store, title, price, lang)
        send_whatsapp_cta(phone, body[:1000], item["link"], bot_id, _view_product_label(lang))
        sent += 1
...
def legacy_text_product_search(product, lang):
    ...
    for primary, secondary in attempts:
        ...
        # بحث أمريكي واحد طبيعي؛ لا توجد تمريرات منفصلة لمتاجر بعينها.
        # أولوية Amazon ثم eBay ثم Walmart تُطبّق فقط عند ترتيب النتائج التي ظهرت فعلاً.
...
def _text_price_local(raw_price, market_rank, lang):
    ...
    if not src:
        src = local_cur if market_rank == 0 else ("USD" if market_rank == 1 else "CNY")
    numeric = _price_decimal(raw)
    if numeric is None: return raw
    if market_rank == 0 and src == local_cur:
        return f"{format_price(numeric, local_cur)} {currency_label(lang)}"
    converted = convert_to_local(numeric, src)
    if converted is None:
        return f"{format_price(numeric, src)} {src}".strip()
    return (
        f"{format_price(converted, local_cur)} {currency_label(lang)} "
        f"({format_price(numeric, src)} {src})"
    )
...
def send_text_lens_style_results(from_number, txt, urls, bot_id, lang, query, exclude_domains=None, exclude_urls=None, more_mode=False):
    ...
    for offer in offers:
        ...
        if not item["link"] or not item["link"].startswith(("http://", "https://")):
            continue
        if item["link"] in exclude_urls or _result_domain(item["link"]) in exclude_domains:
            continue
        rank = result_market_rank(item)
        if rank == 99:
    ...
    selected = _recover_selected_prices(selected, query, lang)
    selected = [item for item in selected if _item_has_price(item)]
    if not selected:
        return False
    ...
    split_cache = [
        (
            title,
            raw_price if _price_decimal(raw_price) is not None
            else str(item.get("price") or "").strip()
        )
    ...
    _save_more_state(from_number, query, bot_id, lang, "text", selected, reset=not more_mode)
    _send_more_results_choice(from_number, bot_id, lang)
...
def _recommendation_pick_search_query(original_query, picked):
    ...
    if not choice: return original
    if not original: return choice
    cleaned = original
    for pat in (
...
def run_brand_comparison(from_number, query, bot_id, lang):
    ...
    for i, o in enumerate(options):
        ...
        rows.append({
            ...
            "description": _pick_description(query, lang),
        })
...
COUNTRY_INPUT_ALIASES = {
 "uk":"gb","united kingdom":"gb","britain":"gb","بريطانيا":"gb",
 "usa":"us","united states":"us","america":"us","أمريكا":"us","امريكا":"us",
 "uae":"ae","emirates":"ae","الإمارات":"ae","الامارات":"ae",
 "saudi arabia":"sa","saudi":"sa","السعودية":"sa","kuwait":"kw","الكويت":"kw",
 "germany":"de","deutschland":"de","ألمانيا":"de","المانيا":"de",
 "france":"fr","فرنسا":"fr","spain":"es","españa":"es","إسبانيا":"es","اسبانيا":"es",
 "italy":"it","italia":"it","إيطاليا":"it","ايطاليا":"it","turkey":"tr","türkiye":"tr","تركيا":"tr",
 "japan":"jp","اليابان":"jp","china":"cn","الصين":"cn","brazil":"br","البرازيل":"br",
}
for _cc,_nm in COUNTRY_NAMES.items():
    COUNTRY_INPUT_ALIASES.setdefault(str(_nm).lower(), _cc)

def resolve_country_input(value):
    raw = re.sub(r"\s+"," ",str(value or "")).strip()
    if re.fullmatch(r"[A-Za-z]{2}", raw):
        cc = raw.lower()
        return cc if cc in COUNTRY_NAMES else ""
    return COUNTRY_INPUT_ALIASES.get(raw.lower(), "")
...
def process_text_message(message,bot_id,onboarding_checked=False):
    ...
    try:
        ...
        _lang_cmd = re.match(r"^\s*(?:language|lang|لغة)\s+([a-zA-Z]{2})\s*$", user_text, flags=re.I)
        if _lang_cmd and _lang_cmd.group(1).lower() in SUPPORTED_LANGUAGES:
            lc = _lang_cmd.group(1).lower()
            USER_LANG[from_number] = lc
            mkt = market_for_user(from_number)
            mkt["language"] = lc
            mkt["locale"] = f"{LANGUAGE_LOCALES.get(lc,lc)}-{mkt['country'].upper()}"
            USER_MARKET[from_number] = mkt
            save_user_preferences(from_number)
            send_whatsapp_text(from_number, f"🌐 {SUPPORTED_LANGUAGES[lc][0]} ✅", bot_id)
            return

        _market_cmd = re.match(r"^\s*(?:market|country|السوق|الدولة)\s*[:=\-]?\s*(.*?)\s*$", user_text, flags=re.I)
        if _market_cmd:
            cc = resolve_country_input(_market_cmd.group(1))
            lang_now = USER_LANG.get(from_number, "ar")
            if not cc:
                send_whatsapp_text(from_number, "ما قدرت أحدد الدولة. مثال: السوق ألمانيا أو market DE." if lang_now=="ar" else "I couldn't identify that country. Try: market Germany or market DE.", bot_id)
                return
            mkt = set_manual_market(from_number, cc)
            send_whatsapp_text(from_number, f"🌍 *{mkt['country_name']}*\n💰 *{mkt['currency']}*", bot_id)
            return
...
def process_location_message(message, bot_id):
    ...
    market = market_for_user(from_number)
    market.update(geo)
    market.update({"lat":lat,"lng":lng,"country_source":"gps"})
    USER_MARKET[from_number]=market
...
@app.get("/")
async def health(): return {"status":"v79 GLOBAL PLUS", "lens_direct_mode":LENS_DIRECT_MODE, "build":BUILD_ID, "location_ttl_hours":LOCATION_TTL_SECONDS//3600, "iso_markets":len(COUNTRY_NAMES), "languages":list(SUPPORTED_LANGUAGES), "affiliate_routing":bool(AFFILIATE_ROUTES)}
